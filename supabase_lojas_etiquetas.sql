...
alter table public.stores
drop constraint if exists stores_daily_goal_positive;

alter table public.stores
add constraint stores_daily_goal_positive check (daily_goal > 0);

alter table public.stores
drop constraint if exists stores_accent_color_valid;

alter table public.stores
add constraint stores_accent_color_valid
...
begin
  ...
  if v_token_column is null then
    raise exception 'Coluna de sessão da loja não encontrada.';
  end if;
...
begin
  ...
  if p_accent_color not in ('blue', 'slate', 'teal', 'green', 'wine', 'amber', 'indigo', 'cyan', 'emerald', 'rose', 'violet', 'orange', 'black') then
  ...
  if v_admin_column is null then
    raise exception 'Coluna de vínculo do admin não encontrada.';
  end if;

  execute format(
    'update public.stores
     set name = $1, daily_goal = $2, accent_color = $3, sort_order = $4
     where id = $5 and %I = $6 and deleted_at is null',
    v_admin_column
  )
...
begin
  ...
  loop
    execute format(
      'update public.stores
       set sort_order = $1
       where id = $2 and %I = $3 and deleted_at is null',
      v_admin_column
    )
...
begin
  ...
  if v_admin_column is null or v_token_column is null then
    raise exception 'Estrutura de loja incompleta.';
  end if;

  execute format(
    'select * from public.stores
     where id = $1 and %I = $2 and deleted_at is null
     limit 1',
    v_admin_column
  )
  into v_store
...
create or replace function public.store_create_tag(p_token text, p_label text)
returns void
language plpgsql
security definer
set search_path = public, extensions
...
begin
  ...
  insert into public.prospect_tags (store_id, label)
  values (v_store_id, v_label)
  on conflict (store_id, label) do nothing;
end;
$$;

grant execute on function public.store_create_tag(text, text) to anon, authenticated;

drop function if exists public.store_get_prospects(text);

create function public.store_get_prospects(p_token text)
...
grant execute on function public.store_get_prospects(text) to anon, authenticated;
...
begin
  ...
  insert into public.prospects (
    store_id, name, phone, cpf, notes, probability, tags, created_at, updated_at
  )
  values (
    v_store_id,
